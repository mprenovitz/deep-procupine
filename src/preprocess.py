"""
This script is adapted from scGPT
"""
...
class Preprocessor:
    ...
    def __init__(
        ...
        use_best_practice_qc: bool = True,
    ):
        r"""
        ...
        filter_cell_by_counts (:class:`int` or :class:`bool`, default: ``False``):
            Whether to filter cells by counts, if :class:`int`, filter cells with counts
        ...
        self.use_best_practice_qc = use_best_practice_qc
    ...
    def __call__(self, adata: AnnData, batch_key: Optional[str] = None) -> Dict:
        ...
        # step 1 through 5 can either be performed using original or best_practice
        if self.use_best_practice_qc:
            run_qc_pipeline(adata)
        else: # run the original scgpt qc pipeline
            self.run_scgpt_run_qc(adata, batch_key, key_to_process)
    ...
    def run_scgpt_run_qc(
        self, adata: AnnData, batch_key: Optional[str] = None, key_to_process: Optional[str] = None
    ):
        """
        Run the original scgpt filtering pipeline. This was refactored into this method,
        so we can run the scrna best_practices.
        """
        # normalize "X" literal to None as done in __call__
        if key_to_process == "X":
            key_to_process = None
